...
namespace SpellWork.Spell
{
    public class SpellInfo
    {
        ...
        public void Write(RichTextBox rtb)
        {
            ...
            {
                foreach (var s in spellByFamily.ToList())
                {
                    var effects = s.SpellEffectInfoStore.ToList();
                    if (!effects.Any(t => (t.SpellEffect.EffectSpellClassMask[0] & SpellClassMask[0]) != 0 ||
                        ...
                        continue;
                    rtb.SetStyle(Color.Blue, FontStyle.Bold);
                    rtb.AppendFormatLine("Modified by {0} ({1})", s.Name, s.ID);
                }
            }
