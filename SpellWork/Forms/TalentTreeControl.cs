...
namespace SpellWork.Forms
{
    ...
    public sealed class TalentTreeControl : Panel
    {
        ...
        {
            var sqlFilePath = TryCreateSqlStub(node);
            var prompt = BuildImplementPrompt(node, currentNotes, sqlFilePath);
        ...
        private string BuildImplementPrompt(TalentNode node, string currentNotes, string? sqlFilePath = null)
        {
            ...
            if (sqlFilePath != null)
                sb.AppendLine($"Place the SQL data in: {sqlFilePath}");
        ...
        private string? TryCreateSqlStub(TalentNode node)
        {
            var repoPath = Settings.Default.RepoPath;
            if (string.IsNullOrWhiteSpace(repoPath))
                return null;

            var sqlBase   = Path.Combine(repoPath, "sql", "updates", "world", "WorldsoulPvP");
            var scriptBase = Path.Combine(repoPath, "src", "server", "scripts", "WorldsoulPvP");
            if (!Directory.Exists(sqlBase) || !Directory.Exists(scriptBase))
                return null;

            var safeClass  = Regex.Replace(_selectedClass.ToLower(),  @"[^a-z0-9]+", "_").Trim('_');
            var safeTalent = Regex.Replace(node.ActiveName.ToLower(), @"[^a-z0-9]+", "_").Trim('_');
            if (string.IsNullOrEmpty(safeClass) || string.IsNullOrEmpty(safeTalent))
                return null;
